...
stories.add('Inline SVG', () => {
  return <Svg height={'30px'} width={'30px'} >{Wishlist}</Svg>;
});

stories.add('Manual SVG Re-use', () => {
  return (
    <div>
      <Svg id={'my-test'} symbol height={'30px'} width={'30px'} >{Wishlist}</Svg>
      <Svg use={'my-test'} height={'30px'} width={'30px'}  />
      <Svg use={'my-test'} height={'30px'} width={'30px'} style={{fill:'red', stroke:'brown'}} />
    </div>
  );
});

stories.add('Auto-SVG Re-use', () => {
  return (
    <SvgProvider>
      <Svg cacheId={'wish-list'} height={'30px'} width={'30px'} >{Wishlist}</Svg>
      <Svg cacheId={'wish-list'} height={'30px'} width={'30px'} style={{fill:'red', stroke:'brown'}} >{Wishlist}</Svg>
    </SvgProvider>
  );
});
